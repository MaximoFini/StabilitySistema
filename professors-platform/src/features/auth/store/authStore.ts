...
// Token expiry: 60 minutes (Supabase handles this, but we track for UI)
            ...
            return {
                ...
                updateActivity: () => {
                    ...
                    inactivityTimer = setTimeout(() => {
                        const state = get()
                        if (state.isAuthenticated) {
                            console.log('Auto-logout: Inactividad detectada')
                            state.logout()
                        }
                    }, INACTIVITY_TIMEOUT)
                },
                ...
                register: async (data) => {
                    ...
                    try {
                        ...
                        if (!professor) {
                            ...
                            if (!professor) {
                                throw new Error('No se pudo obtener el perfil después de crearlo')
                            }
                        }
                ...
                loginWithGoogle: async () => {
                    try {
                        ...
                        const errorMessage = error instanceof Error ? error.message : 'Error con Google OAuth'
                ...
                completeStudentProfile: async (data: StudentProfileData) => {
                    ...
                    try {
                        ...
                        if (data.profileImage) {
                            profileImageUrl = await uploadProfileImage(user.id, data.profileImage)
                        }
                        ...
                        const errorMessage = error instanceof Error ? error.message : 'Error al completar el perfil'
